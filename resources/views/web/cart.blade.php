<x-layout>
    ...
    <div class="container my-5">
        ...
        @if ($cart && count($cart->orderDetails))
            <div class="row">
                <!-- Cart Items -->
                <div class="col-lg-8">
                    <div class="card mb-3">
                        <div class="card-body p-3">
                            @forelse($cart->orderDetails as $item)
                                <div class="cart-item d-flex align-items-center mb-3 border-bottom pb-3">
                                    <img src="{{ optional($item->product)->image_url ? Storage::url(optional($item->product)->image_url) : 'https://via.placeholder.com/80?text=Product' }}"
                                        class="cart-img me-3 rounded" alt="{{ optional($item->product)->name }}"
                                        style="width: 90px; height: 90px; object-fit: cover;">
                                    <div class="flex-grow-1">
                                        <h5 class="cart-item-name mb-1">{{ optional($item->product)->name }}</h5>
                                        <p class="cart-item-price mb-0 text-muted">
                                            Rp.{{ number_format(optional($item->product)->price ?? 0, 0, ',', '.') }}
                                        </p>
                                    </div>
                                    <div class="d-flex align-items-center">
                                        ...
                                        <span
                                            class="cart-item-subtotal mb-0 me-3 fw-semibold">Rp.{{ number_format((optional($item->product)->price ?? 0) * $item->quantity, 0, ',', '.') }}</span>
                    ...
                    <a href="{{ URL::to('/products') }}" class="btn btn-outline-primary mt-2"><i
